...
public class Alignment
{
	...
	public Alignment(String[] names, AlignmentParameters parameters, Aligner aligner)
	{
	...
	public GCGCDatum[][] toArray()
	{
		...
		for (int i = 0; i < rowCount(); i++){tempArray[i] = alignment.get(i).getRow();}
	...
	public AlignmentParameters getParameters(){return params;}
	
	public Aligner getAligner(){return aligner;}
	...
	public void SaveToFile(File f, String separator, SORT_MODE mode) throws IOException
	{
		...
		out.write("Parameters:\n" + params.toString(separator, formatter));
		out.write("\n\n");
		out.write("RT1");
		out.write(separator);
		out.write("RT2");
		...
		for(String st : names){out.write(separator);out.write(st + " Area");}
		...
		for (AlignmentRow row : alignment)
		{
			...
			for (GCGCDatum d : row)
			{
				if (d != null)
				{	                                    
					if(d.getConcentration() > 0 && params.useConcentration()){					
	...
	public List<AlignmentRow> getQuantMassAlignments()
	{
		...
		for(int i = 0; i < alignment.size(); i++)
		{
			...
			if(alignmentRow.getQuantMass() > -1) QuantMassList.add(alignmentRow);
		}
	...
	public int colCount(){return names.length;}
	public int rowCount(){return alignment.size();}
