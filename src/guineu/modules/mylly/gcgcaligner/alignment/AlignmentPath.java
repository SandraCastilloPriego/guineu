...
public class AlignmentPath implements Comparable<AlignmentPath>, Cloneable, Peak {
    ...
    private static AlignmentPath maxPath;
...
//	private long identifiers[];
    ...
    public static AlignmentPath getMaxAlignmentPath() {
        if (maxPath == null) {
            maxPath = new AlignmentPath(0);
            maxPath.score = Double.MAX_VALUE;
        }
        return maxPath;
    }
    ...
    public AlignmentPath(int len, GCGCDatum base, int startCol) {
        this(len);
        this.base = base;
//		this.spectrum = new Spectrum(base.getSpectrum().getPeakList());
    ...
    public boolean containsSame(AlignmentPath anotherPath) {
        ...
        for (int i = 0; i < peaks.length; i++) {
            GCGCDatum d = peaks[i];
            if (d != null && !d.equals(GCGCDatum.getGAP())) {
...
//	{
//		int len = length() - nonEmptyPeaks();
//		int indices[] = new int[len];
//		int k = 0;
//		for (int i = 0; i < this.indices.length; i++)
//		{
//			if (this.indices[i] == NOT_USED)
//			{
//				indices[k++] = i;
//			}
//		}
...
//	}
     ...
     * @return
     */
    public AlignmentPath mergeWith(AlignmentPath other, ScoreCalculator calc, AlignmentParameters params) {
        if (other == null) {
            return clone();
        }
        ...
        for (i = i + 1; i < peaks.length; i++) {
            ...
            if (foundPeak) {
                ...
                combinedPath.addGap(i, params.getGapPenalty());
            }
        }
        return combinedPath;
    }
