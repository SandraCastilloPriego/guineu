...
public class Rules {
	...
	public Rules(SimplePeakListRowGCGC row, List<ComparablePair<Integer, Integer>> spectrumRow, String rules) {
		...
		MiniRule mRule = this.returnRule(rules);
	...
	public Rules() {
		rules = "(Ordinal(57)<=2)&(Ordinal(71)<=2)&(Retention(2)>=1)&(Retention(2)<=1.5)";
	}
	...
	public MiniRule returnRule(String rule) {
		...
		for (Character c : rule.toCharArray()) {
			...
			if (contOpen > 1 && contOpen == contClose) {
				...
				miniRule = new MiniRule(newRule, rule);
				return miniRule;
			}
	...
	public class MiniRule {
		...
		public MiniRule(String ruleString, String oldString) {
			...
			System.out.println("all String: " + oldString);
			try {
				while (oldString.length() > 0) {
					MiniRule rule = returnRule(ruleString);
					if (rule != null) {
						rules.add(rule);
						oldString = oldString.substring(oldString.indexOf(rule.ruleString) + rule.ruleString.length() + 3);

					} else {
						oldString = oldString.substring(ruleString.length() + 3);
					}
					this.getSupraRule();
					System.out.println(oldString);
					MiniRule rule2 = returnRule(oldString);
					if (rule2 != null) {
						rules.add(rule2);
					}
		...
		private void getSupraRule() {
			try {
				String substring = oldString.substring(oldString.indexOf(ruleString), oldString.indexOf(ruleString) + 5);
