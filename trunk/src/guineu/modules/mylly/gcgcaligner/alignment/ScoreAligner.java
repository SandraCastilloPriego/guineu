...
import guineu.modules.mylly.gcgcaligner.gui.AlignmentParameterDialog;
...
import guineu.modules.mylly.gcgcaligner.scorer.QuantMassScoreCalc;
 ...
 * @author jmjarkko
 */
public class ScoreAligner extends AbstractStatusReportingProcess<InputSet, Alignment> implements Aligner
{
	public final static String name = "Spectrum aligner";

	private volatile int peaksTotal;
	private volatile int peaksDone;
	...
	private final ReentrantLock alignmentLock;

	public ScoreAligner(InputSet data, AlignmentParameters params, ScoreCalculator calc)
	{
		this(data, params);
		setScoreCalculator(calc);
	}

	public ScoreAligner(ScoreCalculator calc)
	{
		this((List<GCGCData>) null, AlignmentParameters.getDefaultParameters());
		setScoreCalculator(calc);
	}

	public ScoreAligner()
	{
		this(new SpectrumDotProd());
	}

	public ScoreAligner(InputSet input, AlignmentParameters params)
	{
		this(input != null ? input.getData() : null, params);
	}

	public ScoreAligner(List<GCGCData> dataToAlign, AlignmentParameters params)
	{
		if (params == null)
		{
		...
		if (dataToAlign != null){tempList.addAll(dataToAlign);}
		originalPeakList = java.util.Collections.unmodifiableList(tempList);
		alignmentLock = new ReentrantLock();
	...
	private void copyAndSort(List<GCGCData> dataToAlign)
	{
		...
		if (dataToAlign != null)
		{
			...
			for (int i = 0; i < copyOfData.size(); i++)
			{
				...
				for (GCGCDatum d : peakData){peaksInOneFile.add(d);}
	...
	private List<List<GCGCDatum>> purgeList(List<List<GCGCDatum>> aList)
	{
		...
		for (List<GCGCDatum> l : aList)
		{
			...
			for (GCGCDatum d : l)
			{
				if (d.getSimilarity() >= params.getMinSimilarity() || d.hasQuantMass())
				{
					newSubList.add(d);
				}
			}
	...
	private List<AlignmentPath> generatePaths(ScoreCalculator c, List<List<GCGCDatum>> listOfPeakFiles)
	{
		...
		for (int i = 0; i < listOfPeakFiles.size(); i++)
		{
			...
			while(true)
			{
				...
				if (paths == null){return null;} //Meaning we should stop
				else if (paths.size() == 0){break;}
				/**
				 * TODO See, if this can be done faster without
				...
				reportProgress();
				if (totalAddedPaths == totalPaths){break;}
			}
		}
	...
	private Collection<AlignmentPath> generatePath(int col, ScoreCalculator c, List<List<GCGCDatum>> listOfPeakFiles)
	{
		...
		for (GCGCDatum cur : listOfPeakFiles.get(col))
		{
			...
			if (isCanceled())
			{
				throw new CancellationException("Alignment was cancelled");
			}
			...
			GCGCDatum base, List<List<GCGCDatum>> listOfPeaksInFiles)
	{
		...
		for (int i = (col + 1) % len; i != col; i = (i + 1) % len)
		{
			...
			if (bestPeak != null && bestPeakScore < params.getGapPenalty())
			{
				path.add(i, bestPeak, bestPeakScore);
			}
			else
			{
				path.addGap(i, params.getGapPenalty());
			}
		}
		return path;
	}
	...
	private boolean aligningDone(){return aligningDone;}
	...
	private ThreadInfo[] calculateIntervals(int threads, int col, List<List<GCGCDatum>> listOfPeaks)
	{
		...
		{
			threadInfos[i] = new ThreadInfo(i*diff, ((i == threads-1) ? 
					listOfPeaks.get(col).size() : (i+1)*diff), col);
		}
		return threadInfos;
	}
	
	private void currentStatus(StatusChange.STATUS oldStatus)
	{
		lh.fireStatusChange(new StatusChange(this, oldStatus, currentStatus, ""));
	}

	private void reportProgress()
	{
		lh.fireStatusChange(new StatusChange(this, StatusChange.STATUS.RUNNING,
		                                     StatusChange.STATUS.RUNNING, "progress", 
		                                     peaksDone, peaksTotal));
	}
	...
	{
		alignmentLock.lock();
		StatusChange.STATUS oldStatus = currentStatus;
		currentStatus = StatusChange.STATUS.RUNNING;//We've actually started the thing
		if (alignment == null) //Do the actual alignment if we already do not have the result
		{
			...
			for (AlignmentPath p : addedPaths)
			{
				...
				alignment.addAlignmentRow(p.convertToAlignmentRow().reArrange(originalOrder));
		...
		currentStatus = StatusChange.STATUS.FINISHED;//We're finished
		currentStatus(oldStatus);
	...
	public Aligner getInstance(InputSet dataToAlign,
			AlignmentParameters params)
	{
		ScoreAligner al = new ScoreAligner(dataToAlign, params);
		al.lh = lh.clone();
		al.setScoreCalculator(calc);
		return al;
	}

	public Alignment actualMap(InputSet obj)
	{
		Aligner instance = getInstance(obj, params);
		Alignment result = instance.align();
		checkForCancellation();
		return result;
	}
	...
	{
		final String parameterChangeString = "ParametersFor" + getName() + "haveChanged";
		final AlignmentParameterDialog parameterDialog = new AlignmentParameterDialog(parentWindow, params, parameterChangeString);

		parameterDialog.addPropertyChangeListener(new PropertyChangeListener()
		{
			public void propertyChange(PropertyChangeEvent evt)
			{
				if (evt.getSource() == parameterDialog &&
						parameterChangeString.equals(evt.getPropertyName()))
				{
					params = (AlignmentParameters) evt.getNewValue();
				}
			}	
		});

		parameterDialog.setLocationRelativeTo(parentWindow);
	...
	public ScoreAligner clone()
	{
		ScoreAligner aligner = new ScoreAligner(originalPeakList, params);
		aligner.lh = lh.clone();
		return aligner.setScoreCalculator(calc);
	}
	...
	private class AlignerThread implements Runnable
	{
		...
		public void setThreadInfo(ThreadInfo ti){this.ti = ti;}
...
//			Queue<AlignmentPath> myPaths = new AlignmentPathQueue(512);
